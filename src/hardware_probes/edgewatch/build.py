#!/usr/bin/env python3
"""Build EdgeWatch.ZDL from edgewatch.c + manifest.json (hardware probe).

manifest.json drives the desktop renderer; manifest.json drives this
...
differs). Run from the repo root:  python3 build_all.py edgewatch
"""
...
ROOT = HERE.parent.parent.parent           # src/hardware_probes/edgewatch/build.py -> repo root
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "edgewatch_params.h", "EDGEWATCH")

    src_c = HERE / "edgewatch.c"
    out_dir = ROOT / "dist"
    ...
    print(f"[edgewatch] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
        [
            str(CL6X),
            *CFLAGS,
            f"--define=EDGEWATCH_AUDIO_FUNC={audio_func}",
            "-c",
            str(src_c),
            f"--output_file={obj}",
        ],
    ...
    cfg = LinkerConfig(
        ...
        knob_positions=[(2, 14, 46), (3, 55, 46)],   # 2-knob probe
