#!/usr/bin/env python3
"""Build MatProbe.ZDL from matprobe.c + manifest_pedal.json (7-knob pedal build).
...
differs). Run from the repo root:  python3 build_all.py matprobe
"""
...
ROOT = HERE.parent.parent.parent           # src/custom/matprobe/build.py -> repo root
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "matprobe_params.h", "MATPROBE")

    src_c = HERE / "matprobe.c"
    out_dir = ROOT / "dist"
    ...
    print(f"[matprobe] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
        [
            str(CL6X),
            *CFLAGS,
            f"--define=MATPROBE_AUDIO_FUNC={audio_func}",
            "-c",
            str(src_c),
            f"--output_file={obj}",
        ],
    ...
    cfg = LinkerConfig(
        ...
        knob_positions=[(2, 24, 46), (3, 76, 46)],
