...
ROOT = HERE.parent.parent.parent           # src/custom/stasis/build.py -> repo root
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "stasis_params.h", "STASIS")
    ...
    print(f"[stasis] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
        [
            str(CL6X),
            *CFLAGS,
            f"--define=STASIS_AUDIO_FUNC={audio_func}",
            "-c",
            str(src_c),
            f"--output_file={obj}",
        ],
