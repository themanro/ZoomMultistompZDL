#!/usr/bin/env python3
"""Build Scorch.ZDL from scorch.c + manifest_pedal.json (2-knob amp+cab build)."""
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "scorch_params.h", "SCORCH")
    ...
    print(f"[scorch] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
        [str(CL6X), *CFLAGS, f"--define=SCORCH_AUDIO_FUNC={audio_func}",
