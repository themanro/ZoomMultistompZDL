#!/usr/bin/env python3
"""Build Shatter.ZDL from shatter.c + manifest_pedal.json (2-knob pedal build)."""
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "shatter_params.h", "SHATTER")

    src_c = HERE / "shatter.c"
    out_dir = ROOT / "dist"
    ...
    print(f"[shatter] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
        [str(CL6X), *CFLAGS, f"--define=SHATTER_AUDIO_FUNC={audio_func}",
