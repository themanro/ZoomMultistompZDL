#!/usr/bin/env python3
"""Build Dustbox.ZDL from dustbox.c + manifest_pedal.json (4-knob DOD FX33 clone)."""
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "dustbox_params.h", "DUSTBOX")

    src_c = HERE / "dustbox.c"
    out_dir = ROOT / "dist"
    ...
    print(f"[dustbox] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
        [str(CL6X), *CFLAGS, f"--define=DUSTBOX_AUDIO_FUNC={audio_func}",
    ...
    cfg = LinkerConfig(
        ...
        # 4 knobs (Dust, Motor, Filter, Power): same synthesized-handler
        # setup as corrupt/mangle so knobs 3 and 4 get real edit handlers
        # instead of falling back to NOP_RETURN, which would leave Power
        # unable to update params[] from the physical knob.
