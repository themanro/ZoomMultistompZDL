#!/usr/bin/env python3
"""Build Taffy.ZDL from taffy.c + manifest_pedal.json (2-knob pedal build).
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "taffy_params.h", "TAFFY")
    ...
    print(f"[taffy] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
        [
            str(CL6X),
            *CFLAGS,
            f"--define=TAFFY_AUDIO_FUNC={audio_func}",
            "-c",
            str(src_c),
            f"--output_file={obj}",
        ],
