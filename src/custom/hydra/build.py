#!/usr/bin/env python3
"""Build Hydra.ZDL from hydra.c + manifest_pedal.json (5-knob pedal build).
...
differs). Run from the repo root:  python3 build_all.py hydra
"""
...
ROOT = HERE.parent.parent.parent           # src/custom/hydra/build.py -> repo root
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "hydra_params.h", "HYDRA")
    ...
    print(f"[hydra] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
        [
            str(CL6X),
            *CFLAGS,
            f"--define=HYDRA_AUDIO_FUNC={audio_func}",
            "-c",
            str(src_c),
            f"--output_file={obj}",
        ],
