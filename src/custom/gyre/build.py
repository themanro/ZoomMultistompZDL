#!/usr/bin/env python3
"""Build Gyre.ZDL from gyre.c + manifest_pedal.json (7-knob pedal build).
...
differs). Run from the repo root:  python3 build_all.py gyre
"""
...
ROOT = HERE.parent.parent.parent           # src/custom/gyre/build.py -> repo root
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "gyre_params.h", "GYRE")
    ...
    print(f"[gyre] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
        [
            str(CL6X),
            *CFLAGS,
            f"--define=GYRE_AUDIO_FUNC={audio_func}",
            "-c",
            str(src_c),
            f"--output_file={obj}",
        ],
