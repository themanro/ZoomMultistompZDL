#!/usr/bin/env python3
"""Build Klang.ZDL from klang.c + manifest_pedal.json (2-knob ring-mod build)."""
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "klang_params.h", "KLANG")
    ...
    print(f"[klang] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
        [str(CL6X), *CFLAGS, f"--define=KLANG_AUDIO_FUNC={audio_func}",
