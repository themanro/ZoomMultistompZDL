#!/usr/bin/env python3
"""Build Rewire.ZDL from rewire.c + manifest_pedal.json (7-knob pedal build).
...
differs). Run from the repo root:  python3 build_all.py rewire
"""
...
ROOT = HERE.parent.parent.parent           # src/custom/rewire/build.py -> repo root
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "rewire_params.h", "REWIRE")
    ...
    print(f"[rewire] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
        [
            str(CL6X),
            *CFLAGS,
            f"--define=REWIRE_AUDIO_FUNC={audio_func}",
            "-c",
            str(src_c),
            f"--output_file={obj}",
        ],
