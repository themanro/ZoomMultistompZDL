#!/usr/bin/env python3
"""Build Microlm.ZDL from microloom.c + manifest_pedal.json (2-knob pedal build)."""
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "microloom_params.h", "MICROLOOM")

    src_c = HERE / "microloom.c"
    out_dir = ROOT / "dist"
    ...
    print(f"[microloom] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
        [str(CL6X), *CFLAGS, f"--define=MICROLOOM_AUDIO_FUNC={audio_func}",
