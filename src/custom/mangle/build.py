#!/usr/bin/env python3
"""Build Mangle.ZDL from mangle.c + manifest_pedal.json (2-knob pedal build).
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "mangle_params.h", "MANGLE")
    ...
    print(f"[mangle] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
        [
            str(CL6X),
            *CFLAGS,
            f"--define=MANGLE_AUDIO_FUNC={audio_func}",
            "-c",
            str(src_c),
            f"--output_file={obj}",
        ],
