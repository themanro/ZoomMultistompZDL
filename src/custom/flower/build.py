#!/usr/bin/env python3
"""Build Flower.ZDL from flower.c + manifest_pedal.json (2-knob pedal build).
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "flower_params.h", "FLOWER")
    ...
    print(f"[flower] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
        [
            str(CL6X),
            *CFLAGS,
            f"--define=FLOWER_AUDIO_FUNC={audio_func}",
            "-c",
            str(src_c),
            f"--output_file={obj}",
        ],
