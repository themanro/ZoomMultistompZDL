#!/usr/bin/env python3
"""Build Dissolve.ZDL from dissolve.c + manifest_pedal.json (2-knob pedal build).
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "dissolve_params.h", "DISSOLVE")

    src_c = HERE / "dissolve.c"
    out_dir = ROOT / "dist"
    ...
    print(f"[dissolve] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
        [
            str(CL6X),
            *CFLAGS,
            f"--define=DISSOLVE_AUDIO_FUNC={audio_func}",
            "-c",
            str(src_c),
            f"--output_file={obj}",
        ],
