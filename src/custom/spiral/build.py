#!/usr/bin/env python3
"""Build Spiral.ZDL from spiral.c + manifest_pedal.json (7-knob pedal build).
...
differs). Run from the repo root:  python3 build_all.py spiral
"""
...
ROOT = HERE.parent.parent.parent           # src/custom/spiral/build.py -> repo root
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "spiral_params.h", "SPIRAL")
    ...
    print(f"[spiral] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
        [
            str(CL6X),
            *CFLAGS,
            f"--define=SPIRAL_AUDIO_FUNC={audio_func}",
            "-c",
            str(src_c),
            f"--output_file={obj}",
        ],
