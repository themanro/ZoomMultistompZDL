#!/usr/bin/env python3
"""Build GenLoss.ZDL from genloss.c + manifest_pedal.json (2-knob pedal build)."""
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "genloss_params.h", "GENLOSS")

    src_c = HERE / "genloss.c"
    out_dir = ROOT / "dist"
    ...
    print(f"[genloss] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
        [str(CL6X), *CFLAGS, f"--define=GENLOSS_AUDIO_FUNC={audio_func}",
