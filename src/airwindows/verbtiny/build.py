#!/usr/bin/env python3
"""Build VerbTiny.ZDL from verbtiny.c + manifest.json."""
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "verbtiny_params.h", "VERBTINY")

    src_c = HERE / "verbtiny.c"
    obj = HERE / "verbtiny.obj"
    out_zdl = ROOT / "dist" / f"{manifest['effect_name']}.ZDL"
    out_zdl.parent.mkdir(exist_ok=True)

    print(f"[verbtiny] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
