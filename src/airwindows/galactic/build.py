#!/usr/bin/env python3
"""Build Galactic.ZDL from galactic.c + manifest.json."""
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "galactic_params.h", "GALACTIC")

    src_c = HERE / "galactic.c"
    obj = HERE / "galactic.obj"
    out_zdl = ROOT / "dist" / f"{manifest['effect_name']}.ZDL"
    out_zdl.parent.mkdir(exist_ok=True)

    print(f"[galactic] compiling {src_c.name} -> {obj.name}")
    subprocess.run(
    ...
    cfg = LinkerConfig(
        ...
        screen_image=make_airwindows_reverb_screen("Galactic"),
