...
rebuilt/renamed Airwindows-derived ports, and "contributed" third-party
effects folded in under their own licence. Knob lists here are generated from each
...
| Dustbox | `Dustbox` | Dust, Motor, Filter, Power | contributed | DOD FX33 Buzz Box-style fuzz with a flip-flop sub-octave two octaves down. Dust blends the distortion against the octave branch, Motor is the Grunge gain, Filter is a high-shelf on the distortion only, Power is output level. The divider is fed harmonics rather than a clean tone on purpose, so it free-runs and gets less stable as Motor rises -- the original circuit's chaos rather than a tracked sub-octave. From [marcfuentes/dustbox-zdl](https://github.com/marcfuentes/dustbox-zdl), MIT |
| Rewire | `Rewire` | Route, Bits, Drive, Ring, Comb, Tone, Mix | original | Alesis ModFX Bitrman-inspired lo-fi chain whose blocks can be REORDERED. Four blocks -- bitcrush+decimate, drive, ring mod, comb -- one knob each, and Route permutes them: BDRC, CRDB, DBCR, RCBD, BRDC. The reordering is the point rather than any single block: crush before a comb is a different pedal from a comb before crush, and slot order in a patch reorders whole effects, never stages inside one |
